...
int inst_to_binary(
	...
	// Unconditional Jumps
        ...
        // Question: not sure
        // Answer: its the pos of output - imm
        // imm[19:12]   1111 1111 0000 0000 0000
        binary += ((offset & 0xFF000) << (12 - 12));
        // imm[11]      1000 0000 0000
        binary += ((offset & 0x800) << (20 - 11));
        // imm[10:1]    0011 1111 1111
        binary += ((offset & 0x7FE) << (21 - 1));
	...
	do {
		...
		if (ret != END_OF_FILE && ret != COMMENT_LINE) {
			...
			if (is_opcode(opcode) == LA || is_opcode(opcode) == FILL || is_opcode(opcode) == HALT)
                ...
                //was line and %s, lets see
				printf("[DEBUG] line: %d\tlLabel: %s\n", line_cnt, label);
